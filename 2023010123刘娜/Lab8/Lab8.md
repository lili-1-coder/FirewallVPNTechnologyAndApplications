...
| `wan` 无法回复的原因 |wan路由表中缺少去往10.0.0.0/24网段的回程路由，不知道如何转发ICMP回复包 |
...
答：Lab8中conntrack额外记录了内网真实地址与NAT转换后外网地址的映射关系，即内网主机真实源地址10.0.0.2和防火墙外网口地址192.0.2.1的对应关系，用于防火墙收到外网回复包后，将目的地址还原为内网真实地址。这两个功能都依赖conntrack，说明连接跟踪表（conntrack）是有状态防火墙和NAT功能的底层核心基础，防火墙依靠它实现连接合法性校验与NAT地址转换，是实现网络访问控制和地址映射的关键组件。
2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
答：NAT会为每一条内网发起的连接分配唯一的外网源端口号，即使多台内网主机访问同一个外网服务器的同一端口，外网侧的源端口也各不相同。当外网服务器回复数据包时，防火墙通过查询conntrack连接跟踪表，根据回复包的目的端口匹配对应的内网主机，以此区分不同连接并转发给正确的内网主机。ICMP协议则依靠ICMP的唯一标识ID实现区分，原理与端口号一致。
...
| TCP 38070 访问是否成功 |是 |
| ping 是否成功 |否 |
| ping 失败的原因 |防火墙fw的iptables仅放行TCP 38070 端口的流量，未开放ICMP（ping）协议；防火墙默认转发策略为DROP，因此ping数据包被防火墙拦截丢弃 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有IP地址是内网保留地址，互联网路由器不会转发私有网段的路由，因此公网无法路由到私有地址；同时私有地址可在内网重复使用，公网无法定位唯一主机。NAT将内网私有IP转换为唯一的公网IP，内网主机访问互联网时替换源IP为网关公网IP，依靠连接跟踪表映射回程流量，实现内网主机访问外网。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：SNAT --to-source：适合公网IP固定的场景，性能更高，配置静态公网IP时使用；MASQUERADE：适合公网IP动态变化的场景（如家用宽带拨号），会自动获取出口网卡IP，无需手动指定。本实验公网IP固定，二者效果一致；实际环境固定IP选SNAT，动态IP选MASQUERADE。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：给主动发起连接的协议、P2P协议、FTP、SIP（VoIP）、游戏联机等协议带来麻烦。原因：这类协议会在应用层数据包内部携带内网私有IP/端口，NAT只修改IP首部，不会修改应用层数据；外网收到后无法根据私有IP建立回程连接，导致连接失败。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：conntrack连接跟踪表可以同时实现连接状态记录与NAT地址端口映射。家用路由器需要同时完成内网地址转换上网+防火墙状态检测防护，复用同一张连接跟踪表，简化硬件设计、降低成本，同时提升处理效率，因此二者通常集成。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在防火墙fw命名空间执行，放行 ICMP 新建及相关流量：ip netns exec fw iptables -A FORWARD -p icmp -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
