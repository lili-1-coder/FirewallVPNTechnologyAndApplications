...
| `wan` 无法回复的原因 |已删除 wan 上指向 10.0.0.0/24 的回程路由，wan 不知道如何回复 10.0.0.2 发来的请求，导致 ping 包无法回传 |
...
答：Lab7 中 conntrack 记录连接状态；Lab8 额外记录 NAT 地址映射信息。说明 conntrack 是状态防火墙和 NAT 的共同基础。

2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？

答：NAT 通过 NAPT 机制，用外网端口号区分不同内网主机的连接，实现地址复用。
...
| TCP 38070 访问是否成功 |是 |
| ping 是否成功 |否 |
| ping 失败的原因 |防火墙规则仅允许 TCP 38070 端口的流量通过，未放行 ICMP 协议，因此 ping 包被拒绝 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址（如 10.0.0.0/8）仅在内网有效，互联网路由器不转发目的地址为私有网段的包。NAT 通过将内网私有地址转换为路由器的公网地址，让内网主机可以访问互联网，互联网主机只能看到路由器的公网地址，无法直接访问内网主机。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：本实验中两者效果相同，但实际场景中：
MASQUERADE 适用于动态公网 IP（如家用拨号），会自动适配接口 IP；
SNAT --to-source 适用于固定公网 IP，性能更高，适合服务器场景。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：会给在应用层载荷中携带 IP 地址 / 端口信息的协议（如 FTP、SIP、H.323）带来麻烦。因为 NAT 只修改 IP / 传输层头，不会修改应用层数据，导致载荷中的内网地址无法被外网识别，通信失败。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：conntrack 是两者的共同基础，既能维护连接状态让状态防火墙放行合法回应包，又能记录 NAT 地址映射信息。集成在一起可以共享同一条连接跟踪表，高效处理流量，降低资源消耗。


5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在保留现有规则的基础上，添加允许 ICMP 流量的规则：sudo ip netns exec fw iptables -A FORWARD -p icmp -j ACCEPT
