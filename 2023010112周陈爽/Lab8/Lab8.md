...
| :--- | :--- |
| ping 是否有回应 |无回应 |
| `wan` 收到的请求源地址 |lan 命名空间的内网地址 |
| `wan` 无法回复的原因 |wan 收到的 ICMP 请求源地址是内网私网地址，wan 路由表中没有返回该私网网段的路由，因此无法将响应包发回 lan，导致 ping 失败。 |
...
额外记录的信息：除了连接状态，还记录了 NAT 地址映射关系，包括原始内网地址（10.0.0.2）和转换后的公网地址（192.0.2.1）的对应关系。
说明：这说明 conntrack 是 NAT 和状态防火墙的共同底层支撑，NAT 需要依靠连接跟踪表来还原回包的目的地址，而状态防火墙需要依靠它来判断包是否属于合法连接。两者都依赖 conntrack 维护的连接状态信息。
2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
NAT 会为每一条内网连接分配一个唯一的 公网地址 + 端口映射，不同内网主机的连接，即使访问同一个外网服务器端口，也会被 NAT 映射到不同的公网端口。当外网回复包回来时，NAT 会根据连接跟踪表中的端口映射关系，将包转发到对应的内网主机。
...
| TCP 38070 访问是否成功 | 是|
| ping 是否成功 | 否|
| ping 失败的原因 | 未配置 NAT 规则时，内网主机的 ICMP 请求源地址为私网地址，外网设备无回包路由，无法回复；或 NAT 规则未正确配置，导致地址转换失效，回包无法送达内网主机。|
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：无法直接访问的原因：
私有地址属于内部保留网段，互联网路由器不会为这些网段发布路由，因此外网设备无法将数据包路由到私网地址。
私有地址在不同内网中可重复使用，不具备全局唯一性，互联网无法通过这些地址定位到具体主机。
NAT 的解决方式：
NAT（网络地址转换）在网关设备上将内网主机的私有 IP 地址，转换为网关的公网 IP 地址。内网主机访问外网时，NAT 会修改数据包的源 IP 为网关公网 IP；外网回包时，NAT 再根据连接跟踪表将目的 IP 还原为内网私有 IP，实现内网主机对互联网的访问，同时隐藏内网拓扑。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：SNAT --to-source <固定IP>：适用于静态公网 IP场景，如企业专线。公网 IP 固定不变，使用SNAT性能更高，无需额外维护地址状态，适合长期稳定的网络环境。
MASQUERADE：适用于动态公网 IP场景，如家庭宽带（PPPoE 拨号）。它会自动获取网卡当前的公网 IP，无需手动配置，IP 变化时也能自动适配，适合 IP 不固定的场景。
本实验中网关 IP 固定为192.0.2.1，因此两者效果一致；实际场景中，IP 固定选SNAT，IP 动态选MASQUERADE。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：受影响的协议：FTP、SIP、H.323、RTSP 等，以及 P2P 协议（如 BitTorrent）。
原因：
这些协议的数据包载荷中包含了 IP 地址或端口信息（如 FTP 的主动模式会在载荷中携带数据连接端口），NAT 只会修改 IP 头中的地址，不会修改载荷内的地址信息，导致外网设备无法根据载荷中的内网地址建立连接，从而通信失败。这类协议需要 NAT ALG（应用层网关）支持才能正常工作。


4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：共享连接跟踪表：状态检测防火墙需要conntrack判断数据包是否属于已有合法连接；NAT 需要conntrack记录地址映射关系，两者共享同一张连接跟踪表，集成在一起可以减少资源开销，避免重复维护状态。
协同工作需求：NAT 修改地址后，状态检测需要基于转换后的连接状态进行过滤；状态检测放行的合法连接，NAT 才能进行地址转换，两者依赖同一套状态信息，集成后效率更高、逻辑更清晰。
家用场景简化：家用路由器的 NAT 和防火墙是核心功能，集成在同一模块中可以降低成本、减少配置复杂度，同时保证 NAT 回包能被状态检测规则正确识别和转发。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在 FORWARD 链中，在默认拒绝规则之前添加 ICMP 允许规则，既不影响现有规则，又能让 ping 流量通过。# 1. 允许内网主动发起的 ping 请求（echo-request）
sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-request -j ACCEPT # 2. 允许 ping 的响应包（echo-reply）通过
